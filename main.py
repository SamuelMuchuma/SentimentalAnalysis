...
try:
    conn = mysql.connector.connect(
        host="localhost", user="root", password="", database="sentimentalanalysisdb")
    cursor = conn.cursor()
except:
...
def login_validation():
    ...
    cursor.execute(
        """SELECT * from `logininfo` WHERE `email` LIKE '{}' AND `password` LIKE '{}'""".format(email, password))
...
def add_user():
    ...
    cursor.execute("""INSERT INTO `logininfo` (`name`,`email`,`password`) VALUES ('{}','{}','{}')""".format(
        name, email, password))
    conn.commit()
    cursor.execute(
        """SELECT * from `logininfo` WHERE `email` LIKE '{}'""".format(email))
